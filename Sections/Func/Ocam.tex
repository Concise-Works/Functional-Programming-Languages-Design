...
\begin{Def}[Type Hinting in OCaml]
    ...
    \noindent
    \textbf{Type Hinting for Anonymous Functions:}
    ...
    \noindent
    \rule{\textwidth}{0.4pt}
    \textbf{Typing General Expressions:}\\
...
\newpage 
\subsection{Pattern Matching \& Switch-Case}
...
\subsection{Looping: Recursion, Tail-End Recursion, and Loops}
...
\begin{Def}[Tail-End Recursion]
    ...
    \begin{lstlisting}[language=OCaml, caption={Tail-Recursive Array Summation}, numbers=none]
    let rec sum_arr arr i acc =
        if i <= 0 then acc
        else sum_arr arr (i - 1) (arr.(i - 1) + acc)
    in
    let my_array = [|1; 2; 3; 4; 5|] in
    sum_arr my_array (Array.length my_array) 0
    (* Evaluates to 15 as 1 + 2 + 3 + 4 + 5 *)
    \end{lstlisting}
\end{Def}
...
As you may have seen with \snippet{Array.length <array>}, in OCaml strings must be explicitly manipulated using functions from the 
