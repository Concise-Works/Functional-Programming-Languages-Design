...
\begin{Def}[Mutually Recursive Functions (\texttt{and})]

    Mutually recursive functions are functions that call each other in a cycle. Instead of defining them separately, we use the keyword \snippet{and} to define them simultaneously.
    ...
    \noindent In this example, \snippet{is\_even} calls \snippet{is\_odd} and vice versa. The recursion stops when \( n = 0 \), where we return either \snippet{true} (for even) or \snippet{false} (for odd).
\end{Def}    
\newpage
...
\newpage 
\subsection{Defining Operators: infix \& prefix operators}
\label{subsec:operators}

In OCaml, operators are defined as functions, and we can define our own operators using the \snippet{let} keyword.
\begin{Def}[Infix and Prefix Operators in OCaml]
