...
\begin{Example}[Handling Optional Values When Summing Integers]
    ...
    Suppose we have a function that adds the head of a list to some sum. We might try:
    
    \begin{lstlisting}[language=OCaml, caption={Incorrect: Using [] for an Option}, numbers=none]
    (* Here, suppose next is of some arbitrary strong type *)
    let add_next sum (next: MyType) =
      match next with
      | [] -> sum (* Error: [] is not a valid constructor for int option *)
      | Some x :: _  -> sum + x
    \end{lstlisting}
    ...
    let add_next sum (next: MyType option) =
      match next with
      | None    -> sum         (* Correctly handles the case where there is no integer *)
      | Some x  -> sum + x
    \end{lstlisting}
